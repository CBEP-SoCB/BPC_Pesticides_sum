...
We focus our analysis on bifenthrin.  This Notebook develops a  version
of a key graphic specifically focused on Falmouth, for a presentation to town 
staff.
...
tmp <- conc_data %>%
  filter(! is.na(pct500)) %>%
  select(Location_Descr, pct500, Bifenthrin_ND, Bifenthrin_ML, Bifenthrin_OC_QML) %>%
...
# We pull out coordinates for the Falmouth Points
```{r}
falmouth <- tmp[grepl('Falmouth', tmp$Location_Descr),] %>%
  select(pct500, Bifenthrin_ML)
falmouth
```


### Showing the Resistant Regression
```{r resistant_regression_plot, fig.width=7, fig.height=5}
plt <- ggplot(tmp, aes(pct500, Bifenthrin_ML)) +
  geom_point(aes(color = Bifenthrin_ND), size = 4) +
  geom_point(mapping = aes(color = Bifenthrin_ND), 
             size = 5, data = falmouth, color = 'red') +
...
plt
ggsave('BifenthrinWW_resistant_Falmouth.png', , type='cairo',
